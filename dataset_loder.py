...
class RegressionDataset_with_min_max_scaling(Dataset):
    def __init__(self, h5_path, transforms=None, preload_size=32):
        self.h5_path = h5_path
        self.transforms = transforms
        self.preload_size = preload_size
        self.h5_file = h5py.File(self.h5_path, 'r', libver='latest', swmr=True)
...
class RegressionDataset_with_channel_selector_valid(Dataset):
    def __init__(self, h5_path, selected_channels=None, transforms=None, preload_size=32):
        ...
        self.jet_mult = self.h5_file['jet_mult']
        self.dataset_size = self.data.shape[0]
        self.total_channels = self.data.shape[1]
    ...
    def __getitem__(self, idx):
        ...
        if preload_start != self.preload_start:
            ...
            self.preloaded_jet_mult = self.jet_mult[preload_start:preload_end]
        ...
        jet_mult = self.preloaded_jet_mult[local_idx]
        ...
        return torch.from_numpy(data), torch.from_numpy(labels), torch.from_numpy(iphi), torch.from_numpy(ieta), torch.from_numpy(jet_mass), torch.from_numpy(jet_pt), torch.from_numpy(jet_mult)
...
class RegressionDataset_with_channel_selector_only_jet_info(Dataset):
    def __init__(self, h5_path, selected_channels=None, transforms=None, preload_size=32):
        self.h5_path = h5_path
        self.transforms = transforms
        self.preload_size = preload_size
    ...
    def __getitem__(self, idx):
        ...
        return torch.from_numpy(data), torch.from_numpy(iphi), torch.from_numpy(ieta)
